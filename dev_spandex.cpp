...
void dev_spandex::add_wb(bool& success, addr_breakdown_t addr_br, word_t word, dev_way_t way, hprot_t hprot, bool dcs_en, bool use_owner_pred, cache_id_t pred_cid)
{
    /// try to add an entry
...
void dev_spandex::peek_wb(bool& hit, sc_uint<WB_BITS>& wb_i, addr_breakdown_t addr_br)
{
    ADD_COVERAGE("peek_wb");

    HLS_CONSTRAIN_LATENCY(0, HLS_ACHIEVABLE, "dev-peek wb");
    /// check wb hit
...
void dev_spandex::dispatch_wb(bool& success, sc_uint<WB_BITS> wb_i)
{
    ...
    line_addr = (addr_br.tag << dev_SET_BITS) | (addr_br.set);
...
void dev_spandex::ctrl()
{
    ...
    while(true) {
        ...
        sc_uint<dev_SET_BITS+dev_WAY_BITS>  base = 0;

        dev_rsp_in_t rsp_in;
        dev_fwd_in_t fwd_in;
        dev_cpu_req_t cpu_req;
...

        {
            ...
            /// HLS_CONSTRAIN_LATENCY(0, HLS_ACHIEVABLE, "dev-io-latency");
            ADD_COVERAGE("drain_in_progress");

            can_get_fence_in = (!ongoing_fence) ? dev_fence.nb_can_get() : 0;
            can_get_rsp_in = dev_rsp_in.nb_can_get();
            can_get_req_in = ((dev_cpu_req.nb_can_get() && (!drain_in_progress)) || set_conflict) && !evict_stall && !ongoing_fence && (reqs_cnt != 0); /// if drain in progress, block all cpu requests
            can_get_fwd_in = (dev_fwd_in.nb_can_get() && !fwd_stall) || fwd_stall_ended;
            can_get_flush_in = dev_flush.nb_can_get();
        ...
        if (do_fence)
        {
            ...
            line_breakdown_t<dev_tag_t, dev_set_t> line_br;
            sc_uint<REQS_BITS> reqs_hit_i;
            bool reqs_hit;

            line_br.dev_line_breakdown(rsp_in.addr);
            ...
            if (set_conflict) {
                ...
                {
                    /// eviction
                    line_addr_t line_addr_evict = (tag_buf[evict_way] << dev_SET_BITS) | (addr_br.set);
                    ...
                    if (cpu_req.amo || cpu_req.cpu_msg == READ_ATOMIC || cpu_req.cpu_msg == WRITE_ATOMIC) {
                        ...
                        if (cpu_req.amo) {
                            ...
                            if(amo_hit && sc_success) {
                                ...
                                lines.port1[0][base + way_hit] = line_buf[way_hit];
                                wait();
                                dev_bresp.put(BRESP_EXOKAY);
                            } else {
                                HLS_DEFINE_PROTOCOL("fail_sc");
                                dev_bresp.put(BRESP_OKAY);
                            }
                        ...
                        if(cpu_req.dcs_en && !tag_hit) {
                            ...
                            switch (cpu_req.dcs){
                                case DCS_ReqWTfwd:
                                {
                                    ...
                                    line_addr_t line_addr_inval = (tag_buf[way_hit] << dev_SET_BITS) | (addr_br.set);
...
inline void dev_spandex::reset_io()
{

    /* Reset put-get channels */
    dev_cpu_req.reset_get();
    dev_fwd_in.reset_get();
    dev_rsp_in.reset_get();
    dev_flush.reset_get();
    dev_rd_rsp.reset_put();
    dev_inval.reset_put();
    dev_bresp.reset_put();
    dev_req_out.reset_put();
    dev_rsp_out.reset_put();
#ifdef STATS_ENABLE
    dev_stats.reset_put();
#endif
    dev_fence.reset_get();
    ...
    CACHE_REPORT_INFO("REPORT dev_WAYS");
    CACHE_REPORT_VAR(0, "dev_WAYS", dev_WAYS);
...
void dev_spandex::get_cpu_req(dev_cpu_req_t &cpu_req)
{
    GET_CPU_REQ;

    dev_cpu_req.nb_get(cpu_req);
}

void dev_spandex::get_fwd_in(dev_fwd_in_t &fwd_in)
{
    dev_GET_FWD_IN;


    dev_fwd_in.nb_get(fwd_in);


}

void dev_spandex::get_rsp_in(dev_rsp_in_t &rsp_in)
{
    dev_GET_RSP_IN;

    dev_rsp_in.nb_get(rsp_in);



}

bool dev_spandex::get_flush()
{
    GET_FLUSH;

    bool flush_tmp = false;

    dev_flush.nb_get(flush_tmp);
...
void dev_spandex::send_rd_rsp(line_t line)
{
    SEND_RD_RSP;
    ...
    dev_rd_rsp.put(rd_rsp);
}

void dev_spandex::send_inval(line_addr_t addr_inval, hprot_t hprot_inval)
{
    SEND_INVAL;

    dev_inval_t inval_out;
    inval_out.addr = addr_inval;
    inval_out.hprot = hprot_inval;
    dev_inval.put(inval_out);
}

inline void dev_spandex::send_req_out(coh_msg_t coh_msg, hprot_t hprot, line_addr_t line_addr, line_t line, word_mask_t word_mask)
{
    SEND_REQ_OUT;

    dev_req_out_t req_out;
    ...
    while (!dev_req_out.nb_can_put()) wait();

    dev_req_out.nb_put(req_out);
}

inline void dev_spandex::send_rsp_out(coh_msg_t coh_msg, cache_id_t req_id, bool to_req, line_addr_t line_addr, line_t line, word_mask_t word_mask)
{
    SEND_RSP_OUT;

    dev_rsp_out_t rsp_out;
    ...
    while (!dev_rsp_out.nb_can_put()) wait();

    dev_rsp_out.nb_put(rsp_out);

}

inline void dev_spandex::send_fwd_out(coh_msg_t coh_msg, cache_id_t dst_id, bool to_dst, line_addr_t line_addr, line_t line, word_mask_t word_mask)
{
    SEND_FWD_OUT;

    dev_fwd_out_t fwd_out;
    ...
    while (!dev_fwd_out.nb_can_put()) wait();

    dev_fwd_out.nb_put(fwd_out);
}

#ifdef STATS_ENABLE
void dev_spandex::send_stats(bool stats)
{
    SEND_STATS;
...
void dev_spandex::fill_reqs(cpu_msg_t cpu_msg, addr_breakdown_t addr_br, dev_tag_t tag_estall, dev_way_t way_hit,
		   ...
		   sc_uint<REQS_BITS> reqs_i)
{
    ...
    reqs[reqs_i].invack_cnt  = MAX_N_dev;
...
void dev_spandex::put_reqs(dev_set_t set, dev_way_t way, dev_tag_t tag, line_t line, hprot_t hprot, spx_state_t state,
		  sc_uint<REQS_BITS> reqs_i)
{
    PUT_REQS;

    sc_uint<dev_SET_BITS+dev_WAY_BITS> base = set << dev_WAY_BITS;
...
inline void dev_spandex::read_set(dev_set_t set)
{
...
void dev_spandex::tag_lookup(addr_breakdown_t addr_br, bool &tag_hit, dev_way_t &way_hit, bool &empty_way_found, dev_way_t &empty_way, bool &word_hit)
{
    TAG_LOOKUP;
...
void dev_spandex::reqs_lookup(line_breakdown_t<dev_tag_t, dev_set_t> line_br, sc_uint<REQS_BITS> &reqs_hit_i, bool &reqs_hit)
{
    REQS_LOOKUP;
...
void dev_spandex::reqs_peek_req(dev_set_t set, sc_uint<REQS_BITS> &reqs_i)
{
    REQS_PEEK_REQ;
...
void dev_spandex::reqs_peek_flush(dev_set_t set, sc_uint<REQS_BITS> &reqs_i)
{
    REQS_PEEK_REQ;
...
void dev_spandex::reqs_peek_fwd(addr_breakdown_t addr_br)
{
    REQS_PEEK_REQ;
...
void dev_spandex::self_invalidate()
{
...
void dev_spandex::flush()
{
    ...
    line_addr_t flush_addr = (line_tag << dev_SET_BITS) | (flush_line >> dev_WAY_BITS);
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < N_REQS; ++i) {
            ...
            if (reqs[i].tag == line_tag && reqs[i].set == flush_line >> dev_WAY_BITS && reqs[i].state != SPX_I){
                /// found conflict, cannot dispatch WB
                success = false;
                return;
            }
